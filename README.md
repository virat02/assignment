...
**Prerequisites:** `make`, `docker`, `docker-compose`, `bash`
...
Cache and backing redis is instantiated based on the configurable cache values in [`.env`](#env)
...
For [HTTP server](#http-server):

**`wsgi.py`** 

Runs the flask app on `PROXY_HOST` and `PROXY_PORT` configured via [`.env`](#env) file.
...
For [TCP server](#tcp-server):

**`server.py`**

Listens to and accepts connections from clients connecting on `PROXY_HOST` and `PROXY_PORT` configured via [`.env`](#env) file.
